...
func TestFastIPObfuscatorStatic(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			o, err := NewFastIPv4Obfuscator(schema.ObfuscateReplacementTypeStatic)
...
func TestFastIPObfuscatorConsistent(t *testing.T) {
	for _, tc := range []struct {
		name   string
		input  []string
		output []string
		...
		t.Run(tc.name, func(t *testing.T) {
			o, err := NewFastIPv4Obfuscator(schema.ObfuscateReplacementTypeConsistent)
			require.NoError(t, err)
...
func TestFastIPObfuscationInPaths(t *testing.T) {
	for _, tc := range []struct {
		name   string
		input  string
		output string
